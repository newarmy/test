...
function HuffmanTreeNode (opt) {
    ...
    this.data = opt.data;
    this.binaryFlag = -1; // 左子树为0 or 右子树为1
    this.parent = null; // 为了获得哈夫曼编码
    this.left = null;
    this.right = null;
}
...
function createHuffmanTree(data) {
    ...
    for (let i = 1; i < heapSize; i++) {
        ...
        huffmanTree = new HuffmanTreeNode({weight:0, data: 'node'});
...
// 测试数据
let testData = [
    new HuffmanTreeNode({weight:1, data: 'A'}),
    new HuffmanTreeNode({weight:2, data: 'B'}),
    new HuffmanTreeNode({weight: 8, data: 'F'}),
    new HuffmanTreeNode({weight:21, data: 'U'}),
    new HuffmanTreeNode({weight:12, data: 'P'}),
    new HuffmanTreeNode({weight:5, data: 'O'})];
