...
class Graph {
    ...
    // 从顶点i的最短路径
    unweightedShortestPath (i) {
        let dist = new Array(this.vNum); // 存放最短距离
        let path = new Array(this.vNum); // 存放最短距离路径上的顶点
        ...
        while (!this.queue.isEmptyQ()) {
            let tempNode = this.queue.deleteQ();
            for (let nextNode = tempNode.firstEdge; nextNode; nextNode= nextNode.next ) {
                if(dist[nextNode.adjV] === undefined) {
                    let preVertex = tempNode.vertex || tempNode.adjV;
                    dist[nextNode.adjV] = dist[preVertex]+1;
                    path[nextNode.adjV] = preVertex;
                    this.queue.addQ(this.adjList[nextNode.adjV]);
                }
            }
        }
...
//5-graph.BFS(1);
let pathObj = graph.unweightedShortestPath(1);
function printPath(i) {
    let arr = [];
    arr.push(i);
    while(pathObj.path[i]) {
        arr.push(pathObj.path[i])
        i = pathObj.path[i];
    }
